...
import dk.jonaslindstrom.ruffini.common.matrices.structures.MatrixRing;
...
import static dk.jonaslindstrom.ruffini.common.util.ArrayUtils.nonEmptySubsets;
import static dk.jonaslindstrom.ruffini.common.util.ArrayUtils.removeElements;
...
public class HadamardMatrix {
    ...
    private static List<SamePair<int[]>> divide(int[] set, int d) {
        ...
        return left.map(subset -> new SamePair<>(subset, removeElements(set, subset)))
                .collect(
                        Collectors.toList());
    }
    ...
    private static Matrix<Integer> hadamardMatrixFromTypeI(List<Polynomial<Integer>> A, int n) {
        ...
        List<Matrix<Integer>> m = A.stream().map(p -> p.mapCoefficients(embedding::apply))
                .map(Ai -> Ai.apply(U, matrixRing)).toList();
    ...
    /**
     * For a positive odd <i>k</i>, compute the unique <i>m</i> such that <i>k<sup>2</sup> = (1 +
     * 4m)<sup>2</sup></i>.
     */
    ...
    private static class FormalRing<E> extends PolynomialRingOverRing<E> {
        ...
        public Polynomial<E> multiply(Polynomial<E> a, Polynomial<E> b) {
            Polynomial.Builder<E> result = new Polynomial.Builder<>(super.getRing());
            a.forEachInParallel(
